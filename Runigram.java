...
public class Runigram {
	...
	public static void morph(Color[][] source, Color[][] target, int n) {
		...
		Color[][] tgt = target;
		if (target.length != srcH || target[0].length != srcW) {
			tgt = scaled(target, srcW, srcH);
		}
		for (int step = 0; step <= n; step++) {
			double alpha = (double) (n - step) / n;
			Color[][] frame = blend(source, tgt, alpha);
			display(frame);
			StdDraw.pause(500);
		}
	}
